...
class GoogleSpreadsheet:
    ...
    def __init__(self, credentials_path: str, spreadsheet_id: Optional[str] = None):
        ...
        try:
            ...
            self.logger.debug(f"Google Spreadsheet API接続を初期化しました")
    ...
    def read_sheet(
        self, sheet_name: str, range_name: Optional[str] = None
    ) -> pd.DataFrame:
        """
        ...
        if not self.spreadsheet_id:
            raise ValueError("スプレッドシートIDが設定されていません。set_spreadsheet_id()を使用してIDを設定してください。")
        ...
        try:
            ...
            if not values:
                self.logger.warning(f"シート '{sheet_name}' にデータが見つかりませんでした")
                return pd.DataFrame()
    ...
    def append_sheet(
        ...
        try:
            ...
            self.logger.debug(f"{result.get('updates').get('updatedCells')} セルが追加されました")
            return True
    ...
    def clear_sheet(self, sheet_name: str, range_name: Optional[str] = None) -> bool:
        """
    ...
    def create_spreadsheet(self, title: str, sheet_names: List[str] = ['Sheet1']) -> Optional[str]:
        """
        新しいスプレッドシートを作成する。
        ...
        try:
            ...
            self.logger.debug(f"新しいスプレッドシート '{title}' (ID: {new_spreadsheet_id}) が作成されました")
